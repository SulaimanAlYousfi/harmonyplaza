  ...
  <fieldset style="text-align: center; margin: 10% auto;">
    <form method = "post" action = "login.php">
      ...
      <div class="form-floating center">
        ...
        <br>
      <input type = "submit" name="search" value = "Log In" class = "saveANDclear"/>
      <input type = "reset" value = "Clear" class = "saveANDclear"/>
    </form>

    <br/>
    <a href="signup.php" style="float: left" class="actions">Sign up</a>
    <a href="forgetpass.php" style="float: center" class="actions">Forget Pass</a>
    <a href="deleteuser.php" style="float: right" class="actions">Delete User</a>
  </fieldset>
</body>
<?php
//When button with name "search" is clicked, the following code executes.
  if(isset($_POST['search'])) {
    ...
    $sql = "SELECT * FROM `login` WHERE Email='$email' or `Password`='$password'";
    //Validating user.
    if (mysqli_query($conn, $sql)) {
      mysqli_query($conn, $sql);
    }
    else {
      echo "Connection error. Check Email and Password again." . mysqli_error($conn);
    }
}
